...
export function UnterrichtScreen() {
  ...
  const closeImport = () => {
    if (importPhase === 'processing') { cancelRef.current = true; return }
  ...
  const startProcessing = async (subjectId: string, subjectName: string, folderId: string | undefined) => {
    ...
    for (let i = 0; i < importFiles.length; i++) {
      ...
      try {
        ...
        const { generated, noteTitle } = await analyzeFileToSmartNote(importFiles[i], noteId, subjectName)
        const note: UserNote = {
          id: noteId,
          subjectId: subjectId || undefined,
          folderId: folderId ?? (subjectId ? undefined : 'folder-no-subject'),
          title: noteTitle,
        ...
        setImportFailed((f) => f + 1)
      }
      if (i < importFiles.length - 1 && !cancelRef.current) {
  ...
  const finishImport = () => {
    ...
    if (importSucceeded > 0) {
      if (dest) navigate(`/unterricht/${dest}`)
        ...
        {importPhase === 'processing' && (
          <div className="px-5 pb-5">
            <div className="flex items-center justify-between mb-1">
              <p className="text-[17px] font-bold text-text-primary">KI verarbeitet Dateien</p>
              <p className="text-text-muted text-[13px]">{importCurrent + 1} / {importFiles.length}</p>
            </div>
            ...
            <div className="h-2 bg-border/40 rounded-pill overflow-hidden mb-4">
              <div
                ...
                style={{ width: `${importFiles.length > 0 ? (importCurrent / importFiles.length) * 100 : 0}%` }}
              />
            </div>
            <div className="flex gap-3 mb-5">
              ...
              <div className="flex-1 bg-background border border-border rounded-card px-3 py-2 text-center">
                <p className="text-text-secondary font-bold text-[18px]">{Math.max(0, importFiles.length - importCurrent - 1)}</p>
