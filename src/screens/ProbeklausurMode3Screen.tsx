...
export function ProbeklausurMode3Screen() {
  ...
  async function handleGenerate() {
    ...
    setPhase('loading')
    try {
      const generated = await generateMode3Exam(selectedSubject?.name ?? subjectId, subjectId, topic.trim(), getKc(subjectId) ?? undefined, prefill?.contextText, examEngine)
      setExam(generated)
      setAnswers({})
            ...
            {prefill && (
              <Banner tone="info">
                ...
                <span className="block text-text-secondary mt-0.5">
                  Fach und Thema sind gesetzt, die KI baut Material und Aufgaben auf dem Lernzettel-Inhalt auf. Du kannst unten noch anpassen.
                </span>
              </Banner>
            )}
            <div>
            ...
            <div className="bg-surface rounded-icon border border-border/60 p-4 space-y-3">
              ...
              {[
                ...
                <div key={label} className="flex items-start gap-3">
                  <span className="px-2 py-0.5 rounded-chip text-[11px] font-bold bg-emerald-500/15 text-emerald-400 shrink-0 mt-0.5">{label}</span>
