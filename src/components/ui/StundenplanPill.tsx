...
export function StundenplanPill({ slot, variant, isCurrent, isPast, isNext, style, onClick }: StundenplanPillProps) {
  ...
  if (variant === 'timeline') {
    return (
      ...
      >
        <span className="text-[9px] font-bold leading-tight truncate" style={{ color: accent }}>{icon} {name}</span>
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        <span className="shrink-0" style={{ fontSize: 9 }}>{icon}</span>
  ...
  return (
    ...
    >
      ...
      {isCurrent && (
        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: 'rgba(52,199,89,0.18)', color: '#30D158' }}>Jetzt</span>
      )}
      {isNext && !isCurrent && (
  ...
  if (variant === 'timeline') {
    return (
      <div onClick={onClick} className={`absolute left-0.5 right-0.5 rounded-[7px] flex items-center justify-center gap-1 ${base}`} style={{ ...style, background: bg, borderColor }}>
        <span className="text-[9px] font-semibold truncate" style={{ color: textColor }}>☕ Freistunde</span>
      </div>
    )
  }
