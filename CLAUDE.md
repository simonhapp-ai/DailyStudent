...
1. **Sandbox-Kauftest end-to-end nachholen** — wurde in der Einreichungs-Session nie tatsächlich durchgeführt (StoreKit lieferte vor der Einreichung keine Produktdaten, siehe Session-Notiz). Jetzt nach der Einreichung erneut versuchen. Vorher prüfen: wurde der Sandbox-Tester-Account in App Store Connect (Users and Access → Sandbox → Testers) tatsächlich angelegt? Wurde erklärt, aber nie explizit als erledigt bestätigt.
...
3. **Auf Apples Review-Entscheidung warten** — Status nach Einreichung unbekannt, bei Simon erfragen.
