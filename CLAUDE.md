...
6. **Migrationen 013 + 014 noch nicht angewendet** — `013_lernzettel_modus_images.sql` und `014_abi_pruefungen.sql` liegen im Repo, aber Simon muss beide noch im Supabase SQL Editor ausführen (jeweils unter ihrem Dateinamen ohne `.sql` speichern). Bis dahin schlägt `syncLernzettel()`s bzw. `syncGradeData()`s `upsert()` komplett fehl, weil die jeweiligen Spalten in der echten DB noch fehlen (Postgrest lehnt unbekannte Spalten ab) — landet dann in der bestehenden Sync-Retry-Queue statt verloren zu gehen, aber neue Lernzettel/Abiturprüfungen bleiben bis zur Migration nur lokal (kein Cross-Device). Lokal (localStorage/UI) funktioniert alles normal.
7. **Lernzettel-Erklärbilder (Beta) aktuell komplett funktionsunfähig — Google-seitiges Freikontingent, nicht unser Bug** — direkt gegen Googles API getestet (curl, mit UND ohne Modell-Migration, altes `gemini-2.5-flash-image` UND neues `gemini-3.1-flash-image-preview` zeigen identisch `RESOURCE_EXHAUSTED`/`limit: 0` für `generate_content_free_tier_requests`). Das Freikontingent für Bildgenerierung scheint für dieses Google-Cloud-Projekt aktuell bei 0 zu liegen — passend zu der bereits dokumentierten Sorge, dass das Kontingent knapp werden könnte (siehe Lernzettel-Erklärbilder-Eintrag in Phase 3), nur früher/härter eingetreten als erwartet. Betrifft nur den optionalen Beta-Toggle (default AUS) — der Rest von Lernzettel/Probeklausur/Lernplan/Smart Notes läuft über Text-Modelle und ist nicht betroffen. **Von Simon live in der App bestätigt (25.07.2026):** Toggle „Mit Erklärbildern (Beta)" eingeschaltet, es wird trotzdem kein Bild generiert/angezeigt — kein neues Symptom, deckt sich exakt mit dem curl-Befund. Die Lernzettel-**Textgenerierung** selbst (alle 4 Modi) hat Simon separat live getestet und als gut funktionierend bestätigt („fast gut") — nur dieser Beta-Bildteil ist betroffen. **Wird in einer separaten, parallel laufenden Claude-Code-Session weiterverfolgt** (Google Cloud Billing-Check), nicht Teil dieser Session. **Simon muss:** im Google AI Studio / Google Cloud Billing prüfen, ob für dieses Projekt Billing aktiviert werden muss, um überhaupt wieder Bildgenerierung zu bekommen — reines Model-Downgrade würde das nicht lösen, da beide Modelle betroffen sind.

### To-Do — Priorisiert (Stand: 25.07.2026):

#### Direkt als nächstes (Track B, noch offen aus der 23.–24.07. + 25.07. Session):
✅ ~~Lernzettel-Prompt: Groq → Gemini portieren~~ FERTIG 25.07.2026 — siehe Phase-3-Liste oben. **Von Simon live getestet:** Textgenerierung/Prompts funktionieren gut („fast gut"). **Weiterhin offen:** Erklärbilder-Beta-Toggle erzeugt kein Bild (Google-Freikontingent, siehe Known Issues #7 — wird in paralleler Session weiterverfolgt). **Simon muss noch:** Migration `013_lernzettel_modus_images.sql` im Supabase SQL Editor ausführen (als `013_lernzettel_modus_images` speichern).

✅ ~~Abi-Notenrechner Block II (Abiturprüfungen)~~ FERTIG 25.07.2026 — siehe Phase-3-Liste oben. **Simon muss noch:** Migration `014_abi_pruefungen.sql` im Supabase SQL Editor ausführen (als `014_abi_pruefungen` speichern).

1. **Final QA-Pass auf alle Track-B-Änderungen der 23.–25.07. Sessions** — durchklicken: Google-Login (Account-Picker), Stripe-Checkout + Rabatt-Einlösung (sobald Simon die Coupons angelegt hat), alle 5 neuen Profil-Unterseiten, Übersicht (To-Do-Karte beide Hälften, Streak-Widget, Lernplan-Hero „Fortsetzen", Erste-Schritte-Checkliste, gefächerte Notizen-Karten), Streak-Erklärungs-Sheet, **plus neu:** Lernzettel 4-Modi-Flow + ModusRegler, Erklärbilder-Toggle, Preview-Karussell-Redesign, Markdown/Math-Rendering in Smart Notes + Lernzettel, Abi-Notenrechner Block II.
...
**Migrationen 001–014**, alle in `supabase/migrations/`. 001–012 **alle angewendet** (011 + 012 nachträglich am 24.07.2026). **013 + 014 noch NICHT angewendet — Simon muss beide im Supabase SQL Editor ausführen** (je unter ihrem Dateinamen ohne `.sql` speichern), sonst schlägt das Schreiben von `modus`/`images` (Lernzettel) bzw. `abi_pruefungen` (Notenrechner) beim jeweiligen Sync fehl:
001 initial schema · 002 grade_data · 003 custom_faecher · 004 coins_system · 005 atomic_coins (RPC) · 006 harden_coin_rpcs · 007 note_attachments_storage · 008 early_access · 009 referral_system · 010 personal_entries_extra_fields · 011 coins_discount_redeem (RPC) · 012 ai_rate_limits (RPC) · 013 lernzettel_modus_images (noch offen) · 014 abi_pruefungen (noch offen)
...
supabase/
├── migrations/                    # 001–014, siehe DB-Schema-Sektion oben für Details (013+014 noch offen)
└── functions/
...
**Punkt 2 — Abi-Notenrechner Block II — fertig, siehe Phase-3-Liste oben für Details.** Scoping-Ergebnis mit Simon: kein Bezug zu Niedersachsen-Einbringungsspezifika nötig, sondern die fehlende Grundstruktur ergänzen — Block II (5 Abiturprüfungen, je 0–15 NP ×4, max. 300 Punkte) zusätzlich zu Block I (600 Punkte aus den 4 Halbjahren, unverändert), kombiniert max. 900. Neue Migration `014_abi_pruefungen.sql` — **noch nicht von Simon angewendet**, siehe Known Issues.
